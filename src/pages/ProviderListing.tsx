...
export default function ProviderListing() {
  ...
  const { getProfilesByCategory } = useApp();
  const navigate = useNavigate();
  const [search, setSearch] = useState('');

  const catInfo = CATEGORIES.find(c => c.key === category);
  const providers = getProfilesByCategory(category || '');
      ...
      <div className="px-5 pt-6 pb-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-display text-xl font-bold">
            {catInfo?.icon} {catInfo?.label || 'Providers'}
          </h1>
        </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h3 className="font-semibold">{p.user.firstName} {p.user.lastName}</h3>
                        <p className="text-sm text-muted-foreground">{p.title}</p>
                      </div>
                      <span className="text-sm font-semibold text-primary whitespace-nowrap">{p.priceLabel}</span>
                    </div>
                    <div className="flex items-center gap-3 mt-1 text-sm text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <Star className="h-3.5 w-3.5 fill-eve-gold text-eve-gold" />
                        {p.rating}
                      </span>
                      <span className="flex items-center gap-1">
                    ...
                    <div className="flex gap-1.5 mt-2 flex-wrap">
                      {p.tags.slice(0, 3).map(tag => (
                        <span key={tag} className="text-[10px] font-medium bg-secondary rounded-full px-2 py-0.5">
                          {tag}
                        </span>
                      ))}
                    </div>
